...
export async function enqueueInventoryUpload(uploadJobId: string) { await inventoryQueue.add('process-inventory', { uploadJobId }, { jobId: inventoryBullJobId.csv(uploadJobId) }); }
...
export async function enqueueInventoryImages(uploadJobId: string) { await inventoryQueue.add('process-inventory-images', { uploadJobId }, { jobId: inventoryBullJobId.images(uploadJobId) }); }
