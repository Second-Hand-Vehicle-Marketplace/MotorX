...
export async function listBuyerListings(options: ListBuyerListingsQuery) {
  ...
  const sort: Record<string, SortOrder> = options.sortBy === 'price-asc' ? { price: 1 } : options.sortBy === 'price-desc' ? { price: -1 } : options.sortBy === 'year-desc' ? { year: -1 } : options.sortBy === 'mileage-asc' ? { 'attributes.mileageKm': 1 } : { publishedAt: -1, _id: -1 };
