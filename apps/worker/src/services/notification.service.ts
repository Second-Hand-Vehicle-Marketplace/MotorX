...
import { createNotification, type NotificationChannel } from '../repositories/notification.repository.js';
...
async function notify(userId: Types.ObjectId, type: string, title: string, message: string, channels: NotificationChannel[]) {
  try { await createNotification({ userId, type, title, message, channels }); }
