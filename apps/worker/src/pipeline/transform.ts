...
// Normalizes and validates a batch (against the upload's chosen vehicle category) while
// preserving source rows for correction reports.
export function prepareInventoryBatch(category: VehicleCategory, rows: ExtractedInventoryRow[], firstRowNumber: number) {
  const valid: PreparedInventoryRow[] = []; const invalid: InvalidInventoryRow[] = [];
  rows.forEach((originalData, index) => {
    const rowNumber = firstRowNumber + index; const result = validateInventoryRow(normalizeInventoryRow(category, originalData));
