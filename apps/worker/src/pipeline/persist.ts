...
export async function persistValidRows(dealerId: Types.ObjectId, uploadJobId: Types.ObjectId, rows: Array<{ data: ValidInventoryRow; rowNumber: number }>) {
  ...
  await Promise.all(Array.from({ length: Math.min(8, rows.length) }, async () => {
    while (nextIndex < rows.length) {
      ...
      enrichedRows[index] = { ...row, dealerId, sourceUploadJobId: uploadJobId, sourceRowNumber: rowNumber, images: [], status: 'draft', embedding };
    }
  }));
