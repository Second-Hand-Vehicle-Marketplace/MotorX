...
export function useBuyerListings(initialFilters: ListingFilters = {}, pageSize = 9) {
  ...
  const query = useQuery({ queryKey: ['buyer-listings', filters, page, pageSize], queryFn: () => filters.q ? buyerApi.searchVehicles(filters, page, pageSize) : buyerApi.listVehicles(filters, page, pageSize), placeholderData: (previous) => previous });
  const updateFilters = useCallback((values: Partial<ListingFilters>) => { setFilters((current) => ({ ...current, ...values })); setPage(1); }, []);
