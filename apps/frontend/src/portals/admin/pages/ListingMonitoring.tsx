...
export const ListingMonitoring: React.FC = () => {
  ...
  const loadListings = useCallback(async () => {
    ...
    try {
      const result = await adminApi.listListings({ search: search || undefined, status: status || undefined, category: category || undefined, limit: 100 });
  ...
  return <div>
    ...
    <form className="glass-card" style={{ padding: '1rem 1.5rem', marginBottom: '1.5rem', display: 'flex', gap: '1rem', flexWrap: 'wrap' }} onSubmit={(event) => { event.preventDefault(); setSearch(searchInput.trim()); }}>
      ...
      <select className="form-select" style={{ width: 160 }} value={category} onChange={(event) => setCategory(event.target.value as VehicleCategory | '')}>
        <option value="">All Vehicle Types</option>
        {filterableCategories.map((value) => <option key={value} value={value}>{formatEnumLabel(value)}</option>)}
      </select>
